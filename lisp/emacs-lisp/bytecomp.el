	  ...
	  (let ((code
		 (apply 'make-byte-code
			(append (list arglist)
				...
				(if (or doc int lexical-binding)
				    (list doc))
				;; optionally, the interactive spec.
				(if (or int lexical-binding)
				    (list (nth 1 int)))
				(if lexical-binding
				    '(t))))))
	    (if closure-env
...
(defun byte-compile-closure (form)
  (let ((code (byte-compile-lambda form)))
...
(defun byte-compile-variable-ref (var)
  ...
  (let ((lex-binding (assq var byte-compile-lexical-environment)))
	...
	(if (byte-compile-lexvar-on-stack-p lex-binding)
      ...
      (unless (or (not (memq 'free-vars byte-compile-warnings))
		  (boundp var)
