...
(defmacro ses-set (&rest args)
  "Sets cells values or formulaes.
ARGS is a list of elements chunks to be processed. Each element chunk is
either a setter switch or setting a cell. Possible setters are
identified by 'sqv', 'sv', 'sf', 'sqf', 'sfq', 'sqfq', 'rcv', 'rcf' or
'rcfq'. Default setter id is 'sqv'. Setter can be switched by element
chunk:

:: NEW-SETTER-ID

In NEW-SETTER-ID 's' means symbol, 'rc' means row-column, 's' or 'rc'
tells whether the cell is pointed at by a symbol or by coordinates, with
'0 0' for 'A1'. 'v' means value, and 'f' means formula and this tells
what is assigned to cell.  'q' tells whether the element is implicitely
quoted or not.
